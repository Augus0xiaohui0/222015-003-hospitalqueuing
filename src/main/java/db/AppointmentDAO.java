...
public class AppointmentDAO extends ConnBean {
	...
	public List<AppointmentBean> getAllRecords(){
		//数据库查询的结果集
		ResultSet rs;
		//执行预编译的 SQL 语句，并且可以带有参数，用于向 SQL 查询传递参数
		...
		try {
			...
			//执行 SQL 查询，并将结果存储在 ResultSet 对象 rs 中
	...
	public List<AppointmentBean> getAllRecordsExp(){
		...
		try {
			...
			String sql = "SELECT * FROM appointment WHERE appointment.anum NOT IN (SELECT anum FROM appointment WHERE astate = ?)";
	...
	public int getMaxAnum() {
		...
		try {		
			...
			//创建了一个Statement对象，Statement 对象用于执行静态 SQL 查询并返回其结果
	...
	public boolean backwardRecord(AppointmentBean record,int value) {
		...
		PreparedStatement pstmt=null;
		String sql="update appointment set anum=? where anum=?";
		int result=0; //记录影响的行数
		try{
			connection=getConnection();
			pstmt=connection.prepareStatement(sql); //执行动态SQL语句
			//第一个问号处设置为 record.getAnum()+value
	...
	public List<MedicalTechBean> getAllMRecords() {//医疗设备使用记录
		ResultSet rs=null;
		PreparedStatement pstmt=null;
	...
	public DoctorBean getDInfoByDnum(String dnum) {
		...
		try {		
			...
			pstmt.setString(1, dnum);
			rs=pstmt.executeQuery();
			//rs.next()它的作用是将结果集的指针移动到下一行数据，并返回一个布尔值来指示是否存在下一行。如果返回 true，表示结果集还有下一行数据可以处理；如果返回 false，表示结果集已经处理完了所有数据，没有更多的行了。
			if(rs!=null && rs.next()){
				db=new DoctorBean();
				db.setDnum(dnum);
