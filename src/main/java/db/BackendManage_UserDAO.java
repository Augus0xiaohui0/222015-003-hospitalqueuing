...
public class BackendManage_UserDAO extends BackendManage_ConnBean {
	...
	public List<BackendManage_UserBean> getAllRecords1() {
		ResultSet rs = null;
		Statement stmt = null;
		ArrayList<BackendManage_UserBean> list = new ArrayList<>();
		try {
			connection = getConnection();
			if (connection == null) {
				System.out.println("error");
				return list; // 返回空列表以避免空指针异常
			}
			...
			while (rs.next()) {
				...
				// 使用条件检查确保安全地访问数据库结果集并调用 setter 方法
				user.setDnum(rs.getString(1) != null ? rs.getString(1) : "");
				user.setDname(rs.getString(2) != null ? rs.getString(2) : "");
				user.setDsex(rs.getString(3) != null ? rs.getString(3) : "");
				user.setDage(rs.getInt(4)); // 这里不需要条件检查，getInt() 不会返回 null
				user.setDtel(rs.getString(5) != null ? rs.getString(5) : "");
				user.setDtitle(rs.getString(6) != null ? rs.getString(6) : "");
				user.setDpost(rs.getString(7) != null ? rs.getString(7) : "");
				user.setDedu(rs.getString(8) != null ? rs.getString(8) : "");
				user.setDdepartment(rs.getString(9) != null ? rs.getString(9) : "");
				user.setDcon(rs.getString(10) != null ? rs.getString(10) : "");
				list.add(user);
			}
